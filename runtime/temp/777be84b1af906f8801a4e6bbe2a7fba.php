<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:79:"E:\PHPTutorial\WWW\jjzs\jjzs\public/../application/admin\view\Order\action.html";i:1567051232;s:78:"E:\PHPTutorial\WWW\jjzs\jjzs\public/../application/admin\view\Public\base.html";i:1563256854;}*/ ?>
<!DOCTYPE html>
<html lang="en">
  ...
  <form id="demo-bv-errorcnt" class="form-horizontal bv-form" action="" method="POST" novalidate="novalidate">
    ...
    <div class="panel-body">
      <div class="tab-content">
        ...
        <?php endif; if(\think\Cookie::get('AdminUserType') == 2): if($info['orderStatus'] == 4 || $info['orderStatus'] == 3 || $info['orderStatus'] ==1 && $info['deliverType'] == 2): if($info['huitong']): if(is_array($info['huitong']) || $info['huitong'] instanceof \think\Collection || $info['huitong'] instanceof \think\Paginator): $i = 0; $__LIST__ = $info['huitong'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?>
                  <div class="form-group has-feedback">
                      <label style="" class="col-lg-3 control-label">回桶品牌(数量)</label>
                      <div class="col-lg-4">
                          <input type="text" style="" class="form-control" disabled  name="回桶品牌" value="<?php echo $item['bucket_name']; ?>(<?php echo $item['bucket_num']; ?>)" placeholder="回桶品牌" data-bv-field="name" />

                          <i class="form-control-feedback" data-bv-icon-for="name" style="display: none;"></i>
                      </div>
                  </div>
                <?php endforeach; endif; else: echo "" ;endif; if($info['waterman_owe_bucket'] == 0): ?>
                    <button type="button" class="btn btn-mint disabled" >已回桶</button>
                    <?php else: ?>
                    <button type="button" class="btn btn-mint subHuiTong" >确认回桶</button>
                    <?php endif; endif; endif; endif; if($info['orderTstatus'] == 1): if($info['orderStatus'] == 5): ?>
            <div class="form-group has-feedback">
...
<script>  
    $(function(){
      ...
      $('.completeOrderZiQu').click(function(){
        ...
        $('.subHuiTong').click(function(){
            var type =  $(this).attr('data-type');
            var orderId  =  $(this).attr('data-id');
            $.ajax({
                type: 'POST',
                url:'/Admin/order/ConfirmEmptyBarrey',
                data:{
                    qiantong_order_id:<?php echo $info['qiantong_order_id']; ?>,
                    belong_store:<?php echo $info['belong_store']; ?>,
                    empty_barrey:<?php echo json_encode($info['huitong']); ?>,
                },
                success: function(data){
                    var data= eval('('+data+')');
                    if (data.code == 1) {
