...
class Test extends Command
{
    ...
    {
        $this->setName('Day')->setDescription('计划任务 Day');
    }
    ...
    public function store_task()
    {
        ...
        try{
            ...
            foreach ($branch_store as $key=>$value) {
                ...
                $created_at = time();
                $res = Db::name('revenue_day')->insert(['profit'=>$profit,'expenditure'=>$expenditure,'store_id'=>$value['id'],'statistical_time'=>$statistical_time,'created_at'=>$created_at]);
        ...
        foreach ($branch_store as $key=>$value) {
            ...
            foreach ($order as $K=>$v) {
                ...
                foreach ($order_goods as $keys=>$values) {
                    ...
                    $data['statistical_time'] = strtotime("-1 day");
